<?php
  ...
  $sql = "SELECT Flag, Currency, Rate FROM exchange ORDER BY CASE WHEN Currency = 'PKR' THEN 0 ELSE 1 END, Currency ASC";
...
<header id="header" class="header d-flex align-items-center fixed-top">
  <div class="header-container container-fluid container-xl position-relative d-flex align-items-center justify-content-between">
    ...
    <div class="d-flex align-items-center justify-content-start">
      <a href="https://www.entracloud.net" class="logo d-flex align-items-center">
    ...
    <nav id="navmenu" class="navmenu">
      <ul>
        ...
        <select id="currency-select-desktop" class="form-select d-none d-xl-block">
        
          <?php foreach($xe as $curr) { ?>
          <option value="<?= htmlspecialchars($curr['Currency']) ?>" <?= $curr['Currency'] == 'PKR' ? 'selected' : '' ?>> <?= htmlspecialchars($curr['Flag']) ?> <?= htmlspecialchars($curr['Currency']) ?>
          </option>
          <?php } ?>
    ...
    <div class="d-xl-none d-flex align-items-center ms-auto">
      <select id="currency-select-mobile" class="form-select">
        <?php foreach($xe as $curr) { ?>
          <option value="<?= htmlspecialchars($curr['Currency']) ?>" <?= $curr['Currency'] == 'PKR' ? 'selected' : '' ?>> <?= htmlspecialchars($curr['Flag']) ?> <?= htmlspecialchars($curr['Currency']) ?>
          </option>
        <?php } ?>
      </select>
      <i class="mobile-nav-toggle bi bi-three-dots-vertical"></i>
    </div>
